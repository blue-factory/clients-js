...
class User {
    ...
    get(id) {
        ...
        return __1.promisify(this.client, 'userGet')({ user_id: id });
    }
    getByEmail(email) {
        if (email === undefined) {
            return Promise.reject(new Error('invalid email param'));
        }
        return __1.promisify(this.client, 'userGetByEmail')({ email });
    }
    create(user) {
        if (user === undefined) {
        ...
        return __1.promisify(this.client, 'userCreate')({ data: user });
    }
    verifyPassword(email, password) {
        ...
        return __1.promisify(this.client, 'userVerifyPassword')({ email, password });
    }
}
